...
for (const mode of ['vertex','edge','face']) test(`surface click snaps ${mode} selection at the clicked point under transformed mesh`, async ({page})=>{
  ...
  await page.evaluate(() => { (window as any).__forgeModelingSettings.snapTarget = 'surface'; }); await page.evaluate(() => (window as any).__forgeCommands.vertexSnap()); await page.mouse.click(state.x,state.y);
