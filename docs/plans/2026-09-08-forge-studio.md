...
Status: complete for the supported modeling core; see [Phase 2 completion](2026-09-12-phase2-completion.md) for bevel, quad-loop, multi-object, modifier, UV and worker acceptance boundaries. Component multi-selection, single-triangle extrusion, single-triangle inset and smooth proportional component translation are complete; the remaining modeling-core categories are covered by the completion increment. Connected planar region extrusion is complete; see [region extrusion](2026-09-09-region-extrusion.md) for boundary and planarity scope. Multi-edge subdivision is complete; see [multi-edge subdivision](2026-09-09-multi-edge-subdivision.md) for triangle splitting and [edge subdivision](2026-09-09-edge-subdivision.md) for manifold and attribute constraints. Discrete selection-center snapping to an active-mesh edge midpoint is also implemented; see [edge midpoint snapping](2026-09-12-edge-midpoint-snap.md). Discrete selection-center snapping to an active-mesh vertex is complete; see [vertex snapping](2026-09-09-vertex-snap.md) for scope. Connected proportional translation is also complete; see [connected proportional editing](2026-09-09-connected-proportional.md) for edge-path distance semantics. See [component multi-selection](2026-09-09-component-multiselect.md) for selection behavior, [proportional editing](2026-09-09-proportional-editing.md) for falloff scope, and [triangle inset](2026-09-09-triangle-inset.md) for inset details.
...
Status: collections, bounded texture painting and local texture management are implemented; see [collections](2026-09-14-collections.md), [texture painting](2026-09-14-texture-painting.md) and [texture management](2026-09-14-texture-management.md). Animation now uses nine independent scalar tracks with per-channel key times, Linear/Constant/Bezier segments, Free/Aligned/Auto tangents, direct scalar value editing, per-channel move/Alt-drag copy, channel-key insertion/removal, Blender-style property state colors, and union-frame timeline navigation/export; see [Graph Editor key curves and tangents](2026-09-22-graph-editor-view.md) and [independent channel key times](2026-09-23-independent-channel-key-times.md). The superseded transform-wide key and object/channel interpolation models are rejected on project load. Sculpting, node-based materials and arbitrary F-curves remain open.

## Phase 4: Advanced production

Design a native Forge rigging/skinning system from first principles, then add constraints, geometry nodes, physics, compositing and offline rendering through separately designed backends. The new rig system has no compatibility requirement with the removed preset-specific implementation. Native .blend compatibility is not part of Phase 1; GLB/OBJ are interchange formats. Blender feature parity requires ongoing development and explicit acceptance criteria per subsystem.
...
The initial editor foundation is implemented. The next bounded increment establishes triangle connectivity and component translation before topology-changing operations.
