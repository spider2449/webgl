...
Status: collections, bounded texture painting and local texture management are implemented; see [collections](2026-09-14-collections.md), [texture painting](2026-09-14-texture-painting.md) and [texture management](2026-09-14-texture-management.md). The Graph Editor is the sole animation editing UI and uses a single per-key segment model: Linear by default, with per-key Constant or Bezier overrides, editable Bezier tangents, direct scalar value dragging, whole-transform key move/Alt-drag copy, and playhead display; see [Graph Editor key curves and tangents](2026-09-22-graph-editor-view.md). The superseded object-wide/channel-wide interpolation hierarchy is removed and rejected on project load. Sculpting, node-based materials, independent per-channel key times and arbitrary F-curves remain open.
