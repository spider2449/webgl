...
on('extrude-face', async () => { try {
  ...
  if (!editor.selectedFacesAreTriangles()) throw new Error('Quad / polygon extrusion is the next modeling step; this command currently requires a triangle face.');
  const face = editor.componentFaceTriangles(editor.componentSelection[0])[0];
  await editor.runModeling({kind:'extrude',face,distance:Number($<HTMLInputElement>('#extrude-distance').value)});
...
on('extrude-region', async () => { try {
  ...
  if (!editor.selectedFacesAreTriangles()) throw new Error('Quad / polygon region extrusion is the next modeling step; this command currently requires triangle faces.');
