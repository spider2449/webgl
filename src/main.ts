...
$('#graph-key-tangent').addEventListener('change', event => {
  ...
  try {
    editor.setKeyTangentMode(frame, graphChannel, value);
    updateTimeline();
  } catch (error) {
    toast((error as Error).message);
    updateTimeline();
