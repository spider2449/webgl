...
$('.dialog-note').textContent = 'This release supports vertex, logical edge and face editing, native Cube/Plane quads, polygon-preserving planar extrusion, modeling-core tools, scene collections, Forge-native armature editing, FK/IK posing and basic skinning. General polygon inset, sculpting, physics and native .blend files are planned.';
...
on('extrude-face', async () => { try {
  if (!editor.editMode || editor.componentMode !== 'face' || editor.componentSelection.length !== 1) throw new Error('Select exactly one face in Edit Mode first.');
  const faces = editor.componentFaceTriangles(editor.componentSelection[0]);
...
$('#extrude-face').insertAdjacentHTML('afterend', '<button class="wide-button" id="extrude-region">Extrude planar region</button><p class="field-help">Shift-select connected coplanar logical faces. Uses Extrusion distance and adds walls only along the region boundary.</p>');
on('extrude-region', async () => { try {
  if (!editor.editMode || editor.componentMode !== 'face' || !editor.componentSelection.length) throw new Error('Select connected coplanar faces in Edit Mode first.');
  const faces = editor.componentSelection.flatMap(face => editor.componentFaceTriangles(face));
  await editor.runModeling({kind:'region',faces,distance:Number($<HTMLInputElement>('#extrude-distance').value)});
...
on('inset-face', async () => { try {
  if (!editor.editMode || editor.componentMode !== 'face' || editor.componentSelection.length !== 1) throw new Error('Select exactly one face in Edit Mode first.');
  if (!editor.selectedFacesAreTriangles()) throw new Error('Quad / polygon inset is the next modeling step; this command currently requires a triangle face.');
  const face = editor.componentFaceTriangles(editor.componentSelection[0])[0];
  await editor.runModeling({kind:'inset',face,distance:Number($<HTMLInputElement>('#inset-distance').value)});
