...
$('.dialog-note').textContent = 'This release supports object and vertex editing, Kimodo SOMA77 FK/IK posing and basic skinning. Face modeling, sculpting, physics and native .blend files are planned.';
refreshIcons();
...
function tool(mode: 'select' | 'translate' | 'rotate' | 'scale') {
  if (editor.editMode && mode !== 'select' && mode !== 'translate') { toast('Vertex mode currently supports Move.'); return; }
  activeTool = mode;
  editor.setTool(mode);
...
editor.addEventListener('mode', () => { $<HTMLSelectElement>('#mode').value = editor.editMode ? 'edit' : 'object'; $('#mode-hint').textContent = editor.editMode ? 'Select a vertex, then drag its move gizmo.' : 'Build something extraordinary.'; });
