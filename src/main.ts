import { animationChannels, animationTracks, type AnimationInterpolation } from './animation';
...
const animationGraph = new AnimationGraphView(
  ...
  {
    begin: (frame, channel) => {
      const select = $<HTMLSelectElement>('#animation-channel');
      select.value = channel;
      updateTimeline();
      if (editor.playing) { toast('Pause playback before editing graph keys.'); return false; }
      return editor.beginAnimationKeyDrag(frame);
    },
