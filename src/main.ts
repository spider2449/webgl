...
$('#graph-key-interpolation').addEventListener('change', event => {
  ...
  try {
    editor.setKeyInterpolation(frame, graphChannel, value);
    updateTimeline();
  } catch (error) {
    toast((error as Error).message);
    updateTimeline();
