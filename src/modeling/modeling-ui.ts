...
export function mountModelingUI(editor: Editor, toast: (message: string) => void) {
  ...
  anchor.insertAdjacentHTML('beforebegin', `
    ...
    <details class="modeling-section"><summary>UV editor</summary>
      ...
      <p class="field-help">Select logical faces in the viewport. Quad faces preview both render triangles; the view fits up to 2,000 render triangles. Projection uses local units.</p>
  ...
  action('bevel-edges', () => {
    const edges = requireSelection('edge').map(edge => editor.componentEdgeToTriangleEdge(edge));
    if (edges.some(edge => edge === undefined)) throw new Error('Selected logical edge has no render-edge mapping.');
    return editor.runModeling({ kind: 'bevel', edges: edges as number[], width: value('bevel-width') });
  });
  action('loop-cut', () => {
    const edges = requireSelection('edge');
    if (edges.length !== 1) throw new Error('Select exactly one quad boundary edge.');
    const edge = editor.componentEdgeToTriangleEdge(edges[0]);
    if (edge === undefined) throw new Error('Selected logical edge has no render-edge mapping.');
    return editor.runModeling({ kind: 'loop', edge });
  });
  for (const operation of ['project', 'transform'] as const) action(`uv-${operation}`, () => {
    const faces = requireSelection('face').flatMap(face => editor.componentFaceTriangles(face));
    return editor.runModeling({ kind: 'uv', faces, operation, values: ['uv-u', 'uv-v', 'uv-angle', 'uv-su', 'uv-sv'].map(value) });
  });
  ...
  function drawUV() {
    ...
    const triangleFaces = editor.componentSelection.flatMap(face => editor.componentFaceTriangles(face)).slice(0, 2000);
    const triangles = triangleFaces.map(f => [0, 1, 2].map(j => { const i = geometry.index?.getX(f * 3 + j) ?? f * 3 + j; return [uv.getX(i), uv.getY(i)]; }));
