...
- Edit Mode offers Vertex, Edge and logical Face selection. Forge still renders triangles internally, but native Cube and Plane primitives retain explicit quad cells for modeling: their render diagonals are hidden from Edge selection and either render triangle of a quad selects the same logical face. Imported or otherwise unknown geometry without polygon metadata remains triangle-faced rather than being guessed into quads. Click a component and drag its move gizmo to translate all its logical vertices together; exactly coincident positions move together across normal and UV seams. Vertex and edge selection can reach through the mesh. Use **Extrude selected face** to extrude one planar logical face; a quad cap remains one logical quad while its side walls are created as logical quads. **Extrude planar region** accepts connected coplanar logical faces. Set a positive **Extrusion distance** in local mesh units. Existing UVs/colors and material groups are retained; wall UVs copy boundary values and need later unwrapping. Unsupported attributes, morph targets and partial draw ranges are rejected. Inward extrusion and general polygon inset are not implemented.
...
**Inset selected triangle** remains a triangle-only operation during the polygon-topology transition. It works on triangle-faced meshes (including imported/unknown meshes that have no polygon grouping), but intentionally refuses a native Cube/Plane quad rather than modifying only one hidden render triangle. **Inset distance** is the perpendicular inward distance from each edge in local mesh units and must be smaller than the triangle inradius. The outer boundary stays fixed; the inner triangle remains selected. A true quad / n-gon inset is the next polygon-native modeling step.

### Proportional editing

Enable **Proportional editing** in the Object panel, set a positive **Influence radius**, then move a selected vertex, logical edge or face in Edit Mode. Selected vertices move fully; nearby vertices follow with smooth falloff to zero at the radius. Distance is measured in local mesh units from the nearest selected vertex, including across disconnected geometry. Welded seams stay together. Each drag uses its starting positions and radius; Escape resets the current drag. Geometry changes support undo/redo and project saving. The toggle and radius are session preferences. Enable **Connected only** to measure shortest-path distance along logical modeling edges and keep disconnected islands fixed; hidden render-triangle diagonals of stored quads do not shorten the path. Exact coincident positions still share connectivity across seams. The setting is captured at drag start and remains a session preference. Radius overlays and proportional rotation/scale are not implemented.

### Component multi-selection

In Edit Mode, **Shift-click** to add or remove vertices, logical edges or logical faces.
...
temporary; moved geometry supports undo/redo and project saving. **Extrude selected
face** and **Extrude planar region** operate on logical planar faces; triangle
**Inset** remains explicitly triangle-only until polygon inset is implemented.

### Planar region extrusion

In Edit Mode with Face selection, Shift-click connected coplanar logical faces and choose **Extrude planar region**. The existing **Extrusion distance** sets the positive offset in local units. Each logical face expands to its underlying render triangles for geometry processing, but complete stored polygons remain grouped afterward. The selected faces move together along their common normal; only region boundaries get side walls, including hole boundaries. New side walls are stored as logical quads. Cap faces remain selected for another extrusion or group movement. UV/color seams and material groups are retained; wall UVs inherit boundary coordinates. Undo/redo and Forge projects retain the result.
...
In Edit Mode, select vertices, logical edges or faces and click **Pick snap target** in the Object panel. Choose **Vertex**, **Edge midpoint**, or **Surface point** under **Snap target**. Click an unselected vertex, or a logical edge with both endpoints unselected, in the active mesh to move the unique selection centroid to that target. Surface targets still use the actual clicked render triangle so barycentric placement remains precise; all three target triangle vertices must be unselected. Rotation and non-uniform object scale are supported. Hidden render diagonals of stored quads are not Edge targets; edge guides show the same logical boundaries used by Edge selection. Selected vertices retain their relative spacing and welded seam copies move together; unselected geometry stays fixed. Picking can reach through geometry, as with vertex selection. Empty or invalid-target clicks keep the action active. Escape or **Cancel snap target** cancels without changing geometry. Switching modes or target kinds also cancels.
...
| UV editing | Select logical faces, open **UV editor**, project UVs or translate/rotate/scale existing UVs around their selected-corner center. Stored quads expand to both render triangles for UV processing while retaining their logical grouping; the canvas previews up to 2,000 render triangles. |
...
The editor now has an initial logical polygon editing layer for native Cube/Plane quads and polygon-preserving planar extrusion. It does not yet include general polygon inset, automatic polygon reconstruction for arbitrary imports, polygon-preserving output for every topology-changing tool, curved-surface region extrusion, sculpting, weight painting, IK pole vectors/joint limits, retargeting, geometry nodes, physics, compositing or offline rendering. The modeling core includes bevel, loop cuts, UV editing, modifiers and mesh snapping within the supported limits documented above. Kimodo text-to-motion inference is not connected. The UI exposes only implemented local workflows and labels the basic rigging limitations.
...
Left-drag in the 3D viewport draws a selection marquee. Object Mode selects objects whose projected bounds overlap the box; Edit Mode selects logical vertex positions, logical screen-space edge segments, or logical face centroids according to the active component type. Weight Mode uses the same vertex box selection. Hold Shift while dragging to add to the current selection. Hold Ctrl while dragging to toggle every boxed item: selected items are removed and unselected items are added. Ctrl takes precedence over Shift for a marquee started with both modifiers. Press Escape to cancel an active marquee. Alt-left orbit remains separate from box selection.
...
Forge still renders triangles internally. Native Cube and Plane geometry now stores a separate logical polygon map, so each grid cell is edited as a quad while its two render triangles remain an implementation detail. In Edit Mode, Wireframe hides the selected mesh's raw triangle wireframe and draws the same logical boundary edges used for selection, so an internal quad diagonal is not presented as an unselectable modeling edge. That polygon metadata survives primitive regeneration, Forge project serialization, UV edits, vertex movement and planar extrusion. Geometry without trusted polygon metadata is intentionally edited as triangles rather than heuristically merging faces. Bevel, loop cut, subdivision and other topology-changing tools are still being migrated to emit polygon metadata consistently.
